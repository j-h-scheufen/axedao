...
const addLiquidity = async (
  ...
  const tx = await router.connect(owner).addLiquidity(axe, dai, axeAmount, daiAmount, 0, 0, owner.address, deadline);
  await tx.wait();
};
...
const removeLiquidity = async (
  ...
  const deadline = Math.floor(Date.now() / 1000) + 10 * 60; // 10 min
  const tx = await router.connect(owner).removeLiquidity(axe, dai, liquidity, 0, 0, owner.address, deadline);
  await tx.wait();
};
