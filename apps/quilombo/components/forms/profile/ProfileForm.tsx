...
const ProfileForm = () => {
	...
	const requestEmail = useCallback(async (): Promise<string | undefined> => {
		const silk = connectors.find(
			(connector: { id: string }) => connector.id === "silk",
		);
		if (silk) {
						...
						<div className="flex flex-col gap-2 sm:gap-4">
							...
							<div className="flex w-full items-center gap-2">
								<Field
									...
									as={FieldInput}
									startContent={
										<Mail className="pointer-events-none h-4 w-4 flex-shrink-0 text-default-400" />
									}
									disabled
								/>
							...
							<Field
								...
								startContent={
									<Phone className="pointer-events-none h-4 w-4 min-w-4 flex-shrink-0 text-default-400" />
								}
							/>
						</div>
