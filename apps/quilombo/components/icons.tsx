...
import { LayoutPanelTop, type LucideIcon } from 'lucide-react';
import { cn } from '@/utils/tailwind';

const Logo: React.FC<IconSvgProps> = ({ size = 36, width, height, ...props }) => {
  const id = useId();
  return (
    <svg fill="none" height={size || height} viewBox="0 0 32 32" width={size || width} {...props} aria-labelledby={id}>
      <title id={id}>Logo</title>
      <path
        clipRule="evenodd"
...
const DiscordIcon: React.FC<IconSvgProps> = ({ size = 24, width, height, ...props }) => {
  const id = useId();
  return (
    <svg height={size || height} viewBox="0 0 24 24" width={size || width} {...props} aria-labelledby={id}>
      <title id={id}>Discord</title>
      <path
