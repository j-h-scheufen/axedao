...
const LocationModal: React.FC<LocationModalProps> = ({ initialFeature, onConfirm, ...modalProps }) => {
  const [selectedFeature, setSelectedFeature] = useState<MaptilerFeature | null>(initialFeature);
  const [coordinates, setCoordinates] = useState<[number, number] | null>(
    initialFeature?.geometry?.type === 'Point' ? (initialFeature.geometry.coordinates as [number, number]) : null
  );
