 ...
 *   get:
 *     summary: Check if group can be verified
 *     description: |
 *       Returns whether the group can currently be verified based on the 30-day cooldown period.
 *     tags:
 *       - Groups
 *     security:
 *       - session: []
 ...
 *     responses:
 *       200:
 *         description: Cooldown status returned successfully
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 */
export async function GET(_request: NextRequest, { params }: RouteParamsGroup) {
  ...
  try {
    ...
    // Check if group can be verified
    ...
    if (canVerify) {
      ...
      return NextResponse.json({
        canVerify: false,
        message: 'This group was recently verified. Groups can only be verified once every 30 days.',
      });
    }
  } catch (error) {
    console.error('Error checking verification cooldown:', error);
    return NextResponse.json(
      { error: generateErrorMessage(error, 'An unexpected error occurred while checking verification status') },
      { status: 500 }
    );
  }
}
