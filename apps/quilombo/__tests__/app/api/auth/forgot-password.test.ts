 ...
 *
 * Tests the forgot password endpoint with mocked database and email.
 * Verifies validation, rate limiting, token generation, and security (no user enumeration).
 */
...
describe('POST /api/auth/forgot-password', () => {
  ...
  describe('Successful Password Reset Flow', () => {
    it('should delete existing unconsumed reset tokens before creating new one', async () => {
    ...
    it('should generate and store password reset token', async () => {
      ...
      expect(hashToken).toHaveBeenCalledWith('reset-token-123');
