#!/usr/bin/env tsx
/**
 * Staging Database Migration Runner
 *
 * This script runs database migrations on the staging environment with safety features:
 ...
 *
 * Usage:
 *   tsx scripts/db-migrate-staging.ts
 *
 ...
 * Lock ID is derived from a hash of 'quilombo_staging_migration'
 */
async function acquireLock(): Promise<boolean> {
  try {
    const lockId = 'quilombo_staging_migration';
...
async function rollback(beforeState: MigrationRecord[]): Promise<void> {
  ...
  log.info('\nRollback steps:');
  log.info('1. Connect to your staging Supabase database using psql or the Supabase SQL editor');
  ...
  log.error('\n⚠️  DO NOT MERGE TO MAIN UNTIL ROLLBACK IS COMPLETE ⚠️');
}
