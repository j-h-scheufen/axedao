...
export async function POST(request: NextRequest, { params }: { params: { groupId: string } }) {
  ...
  try {
    ...
    // Image processing
    const imageBuffer = await sharp(buffer).resize(IMAGE_FORMATS.groupLogo).webp({ lossless: true }).toBuffer();
    ...
    const IpfsHash = await axios
      ...
      .catch((error) => {
        ...
        return NextResponse.json({ error: 'Unable to save group logo' }, { status: 500 });
      });

    // Delete existing logo file before updating DB
    if (group.logo) {
      await axios
        .delete(`https://api.pinata.cloud/pinning/unpin/${group.logo}`, {
          headers: {
            Authorization: `Bearer ${process.env.PINATA_JWT}`,
          },
        })
        .catch((error) => {
          ...
          // Don't return an error here, as the group's logo has been updated successfully
        });
    }

    const updatedGroup = await updateGroup({ ...group, logo: IpfsHash });
